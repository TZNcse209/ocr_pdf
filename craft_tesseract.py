...
def text_detection_and_draw_boxes_craft(image_path, output_folder):
    ...
    result = craft.detect_text(image_rgb)

    for box in result['boxes']:
        # Draw bounding boxes
        cv2.polylines(input_image, [box.astype(np.int32).reshape((-1, 1, 2))], isClosed=True, color=(0, 255, 0), thickness=2)

    text_detection_image_path = os.path.join(output_folder, f"text_detection_craft_{os.path.basename(image_path)}")
...
def merge_nearby_boxes_and_text_recognition(image_path, output_folder):
    ...
    output_txt_path = os.path.join(output_folder, f'recognized_text_{os.path.basename(image_path)}.txt')
    ...
    with open(output_txt_path, 'w') as txt_file:
        for block_id in merged_block_ids:
            ...
            text = pytesseract.image_to_string(segment_image, config='--psm 6', output_type=Output.STRING)
...
def process_pdf_files(pdf_folder, output_folder):
    ...
    for image_file in image_files:
        ...
        text_detection_and_draw_boxes_craft(image_path, output_folder)
